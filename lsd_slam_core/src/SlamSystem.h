...
class SlamSystem {
 ...
 public:
  ...
  /** Sets the visualization where point clouds and camera poses will be sent
   * to. */
 ...
 private:
  ...
  std::vector<Frame*, Eigen::aligned_allocator<Frame*>> KFForReloc;
  int nextRelocIdx;
  ...
  std::shared_ptr<Frame>
      trackingReferenceFrameSharedPT;  // only used in odometry-mode, to keep a
                                       // keyframe alive until it is deleted.
                                       // ONLY accessed whithin
  ...
  // GUARANTEED to give the same result each call, and to be compatible to each
  // other.
