...
DepthMap::~DepthMap() {
  if (activeKeyFrame != 0) activeKeyFramelock.unlock();
...
void DepthMap::observeDepth() {
  threadReducer.reduce(
      boost::bind(&DepthMap::observeDepthRow, this, _1, _2, _3), 3, height - 3,
      10);
...
bool DepthMap::observeDepthCreate(const int& x, const int& y, const int& idx,
  ...
  if (refFrame->getTrackingParent() == activeKeyFrame) {
    ...
    if (wasGoodDuringTracking != 0 &&
      ...
      if (plotStereoImages)
        debugImageHypothesisHandling.at<cv::Vec3b>(y, x) =
            cv::Vec3b(255, 0, 0);  // BLUE for SKIPPED NOT GOOD TRACKED
      return false;
    }
  }
  ...
  float error = doLineStereo(
      new_u, new_v, epx, epy, 0.0f, 1.0f, 1.0f / MIN_DEPTH, refFrame,
  ...
  if (error < 0 || result_var > MAX_VAR) return false;
  ...
  *target = DepthMapPixelHypothesis(result_idepth, result_var,
  ...
  if (plotStereoImages)
    debugImageHypothesisHandling.at<cv::Vec3b>(y, x) =
        cv::Vec3b(255, 255, 255);  // white for GOT CREATED
...
bool DepthMap::observeDepthUpdate(const int& x, const int& y, const int& idx,
  ...
  float error = doLineStereo(
      x, y, epx, epy, min_idepth, target->idepth_smoothed, max_idepth, refFrame,
      refFrame->image(0), result_idepth, result_var, result_eplLength, stats);
  ...
  // minimum; ..)
    ...
    if (plotStereoImages)
      debugImageHypothesisHandling.at<cv::Vec3b>(y, x) =
          cv::Vec3b(255, 0, 255);  // PURPLE FOR NON-GOOD
  ...
  // if not found (error too high)
    ...
    if (plotStereoImages)
      debugImageHypothesisHandling.at<cv::Vec3b>(y, x) =
          cv::Vec3b(0, 0, 0);  // BLACK FOR big not-found
    ...
    if (plotStereoImages)
      debugImageHypothesisHandling.at<cv::Vec3b>(y, x) =
          cv::Vec3b(0, 0, 0);  // BLACK FOR big arithmetic error
  ...
  // if inconsistent
    ...
    if (plotStereoImages)
      debugImageHypothesisHandling.at<cv::Vec3b>(y, x) =
          cv::Vec3b(255, 255, 0);  // Turkoise FOR big inconsistent
    ...
    id_var = id_var * w;
    if (id_var < target->idepth_var) target->idepth_var = id_var;
    ...
    if (result_eplLength < MIN_EPL_LENGTH_CROP) {
      ...
      if (result_eplLength < 0.5 * MIN_EPL_LENGTH_CROP) inc *= 3;
    ...
    if (plotStereoImages)
      debugImageHypothesisHandling.at<cv::Vec3b>(y, x) =
          cv::Vec3b(0, 255, 255);  // yellow for GOT UPDATED
...
void DepthMap::propagateDepth(Frame* new_keyframe) {
  ...
  if (new_keyframe->getTrackingParent() != activeKeyFrame) {
    printf(
        "WARNING: propagating depth from frame %d to %d, which was tracked on "
        "a different frame (%d).\nWhile this should work, it is not "
        "recommended.",
  ...
  for (int y = 0; y < height; y++)
    for (int x = 0; x < width; x++) {
      ...
      if (!targetBest->isValid) {
        ...
        *targetBest = DepthMapPixelHypothesis(new_idepth, new_var,
  ...
  if (enablePrintDebugInfo && printPropagationStatistics) {
    printf(
        "PROPAGATE: %d: %d drop (%d oob, %d color); %d created; %d merged; %d "
        "occluded. %d col-dec, %d grad-dec.\n",
        ...
        runningStats.num_prop_removed_out_of_bounds,
        runningStats.num_prop_removed_colorDiff, runningStats.num_prop_created,
        runningStats.num_prop_merged, runningStats.num_prop_occluded,
  ...
  for (int y = yMin; y < yMax; y++) {
    for (int x = 3; x < width - 2; x++) {
      ...
      if ((dest->blacklisted >= MIN_BLACKLIST &&
        ...
        currentDepthMap[idx] =
            DepthMapPixelHypothesis(idepthObs, VAR_RANDOM_INIT_INITIAL, 0);
...
void DepthMap::buildRegIntegralBufferRow1(int yMin, int yMax,
  ...
  for (int y = yMin; y < yMax; y++) {
    ...
    for (int x = 0; x < width; x++) {
      if (ptSrc->isValid) validityIntegralBufferSUM += ptSrc->validity_counter;
      ...
      for (int dx = -regularize_radius; dx <= regularize_radius; dx++)
        for (int dy = -regularize_radius; dy <= regularize_radius; dy++) {
          ...
          if (DIFF_FAC_SMOOTHING * diff * diff >
            ...
            if (removeOcclusions) {
              if (source->idepth > destRead->idepth) numOccluding++;
            }
            continue;
          }
          ...
          if (removeOcclusions) numNotOccluding++;
...
void DepthMap::regularizeDepthMap(bool removeOcclusions, int validityTH) {
  ...
  if (enablePrintDebugInfo && printRegularizeStatistics)
    printf(
        "REGULARIZE (%d): %d smeared; %d blacklisted /%d new); %d deleted; %d "
        "occluded; %d filled\n",
    ...
    for (int x = 0; x < width; x++) {
      if (*idepthVar > 0) {
        *pt = DepthMapPixelHypothesis(*idepth, *idepthVar, *validity);
        ...
        currentDepthMap[x + y * width].isValid = false;
        currentDepthMap[x + y * width].blacklisted =
...
void DepthMap::updateKeyframe(
  ...
  for (std::shared_ptr<Frame> frame : referenceFrames) {
    ...
    if (frame->getTrackingParent() != activeKeyFrame) {
      printf(
          "WARNING: updating frame %d with %d, which was tracked on a "
          "different frame (%d).\nWhile this should work, it is not "
          "recommended.",
          activeKeyFrame->id(), frame->id(), frame->getTrackingParent()->id());
    }
  ...
  if (plotStereoImages) {
    ...
    cv::Mat newest_refImage(
        newest_referenceFrame->height(), newest_referenceFrame->width(), CV_32F,
  ...
  msUpdate = 0.9 * msUpdate +
             0.1 * ((tv_end_all.tv_sec - tv_start_all.tv_sec) * 1000.0f +
  ...
  if (enablePrintDebugInfo && printLineStereoFails) {
    printf(
        "ST-ERR: oob %d (scale %d, inf %d, near %d); err %d (%d uncl; %d end; "
        "zro: %d btw, %d no, %d two; %d big)\n",
...
void DepthMap::createKeyFrame(Frame* new_keyframe) {
  ...
  if (plotStereoImages) {
    // Util::displayImage( "KeyFramePropagation",
  ...
  if (sPassed > 1.0f) {
    ...
    if (enablePrintDebugInfo && printMappingTiming) {
      printf(
          ...
          msUpdate, nAvgUpdate, msCreate, nAvgCreate, msFinalize, nAvgFinalize,
          msObserve, nAvgObserve, msRegularize, nAvgRegularize, msPropagate,
          nAvgPropagate, msFillHoles, nAvgFillHoles, msSetDepth, nAvgSetDepth);
    }
  }
}
...
inline float DepthMap::doLineStereo(
  ...
  float realVal_p1 =
      getInterpolatedElement(activeKeyFrameImageData, u + epxn * rescaleFactor,
  ...
  float realVal_m2 = getInterpolatedElement(
      activeKeyFrameImageData, u - 2 * epxn * rescaleFactor,
  ...
  if (isnanf((float)(pFar[0] + pClose[0]))) return -4;
   ...
   * sum(e_i*e_i),sum(e_{i-1}*e_{i-1}),sum(e_{i+1}*e_{i+1})
  ...
  if (useSubpixelStereo) {
    ...
    if (enablePrintDebugInfo &&
        (best_match_errPre < 0 || best_match_errPost < 0)) {
  ...
  if (idnew_best_match < 0) {
    ...
    if (!allowNegativeIdepths) return -2;
  }
