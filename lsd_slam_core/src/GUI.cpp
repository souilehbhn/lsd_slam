...
GUI::GUI() : depthImg(0), depthImgBuffer(0) {
  //    pangolin::CreateGlutWindowAndBind("Main", 1280 + 180, 960, GLUT_DOUBLE |
  //    GLUT_RGBA | GLUT_DEPTH | GLUT_MULTISAMPLE);
...
void GUI::initImages() {
  depthImg = new pangolin::GlTexture(Resolution::getInstance().width(),
                                     Resolution::getInstance().height(), GL_RGB,
                                     true, 0, GL_RGB, GL_UNSIGNED_BYTE);

  depthImgBuffer.assignValue(
      new unsigned char[Resolution::getInstance().numPixels() * 3]);
}

void GUI::updateImage(unsigned char *data) {
  boost::mutex::scoped_lock lock(depthImgBuffer.getMutex());

  memcpy(depthImgBuffer.getReference(), data,
...
void GUI::drawFrustum() {
  ...
  glBegin(GL_LINES);
  glVertex3f(0, 0, 0);
  glVertex3f(0.05 * (0 - Intrinsics::getInstance().cx()) /
                 Intrinsics::getInstance().fx(),
             0.05 * (0 - Intrinsics::getInstance().cy()) /
                 Intrinsics::getInstance().fy(),
             0.05);
  glVertex3f(0, 0, 0);
  glVertex3f(0.05 * (0 - Intrinsics::getInstance().cx()) /
                 Intrinsics::getInstance().fx(),
             0.05 * (Resolution::getInstance().height() - 1 -
                     Intrinsics::getInstance().cy()) /
                 Intrinsics::getInstance().fy(),
             0.05);
  glVertex3f(0, 0, 0);
  glVertex3f(0.05 * (Resolution::getInstance().width() - 1 -
