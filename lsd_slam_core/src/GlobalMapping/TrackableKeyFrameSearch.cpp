...
TrackableKeyFrameSearch::TrackableKeyFrameSearch(KeyFrameGraph* graph, int w,
                                                 int h, Eigen::Matrix3f K)
    : graph(graph) {
...
TrackableKeyFrameSearch::~TrackableKeyFrameSearch() { delete tracker; }

std::vector<TrackableKFStruct, Eigen::aligned_allocator<TrackableKFStruct>>
TrackableKeyFrameSearch::findEuclideanOverlapFrames(Frame* frame,
                                                    float distanceTH,
                                                    float angleTH,
  ...
  Eigen::Vector3d viewingDir =
      frame->getScaledCamToWorld().rotationMatrix().rightCols<1>();

  std::vector<TrackableKFStruct, Eigen::aligned_allocator<TrackableKFStruct>>
      potentialReferenceFrames;
...
Frame* TrackableKeyFrameSearch::findRePositionCandidate(Frame* frame,
                                                        float maxScore) {
  std::vector<TrackableKFStruct, Eigen::aligned_allocator<TrackableKFStruct>>
      potentialReferenceFrames = findEuclideanOverlapFrames(
          frame, maxScore / (KFDistWeight * KFDistWeight), 0.75);
  ...
  for (unsigned int i = 0; i < potentialReferenceFrames.size(); i++) {
    if (frame->getTrackingParent() == potentialReferenceFrames[i].ref) continue;

    if (potentialReferenceFrames[i].ref->idxInKeyframes <
        INITIALIZATION_PHASE_COUNT)
      continue;

    struct timeval tv_start, tv_end;
    gettimeofday(&tv_start, NULL);
    tracker->checkPermaRefOverlap(potentialReferenceFrames[i].ref,
    ...
    if (score < maxScore) {
      SE3 RefToFrame_tracked =
          tracker->trackFrameOnPermaref(potentialReferenceFrames[i].ref, frame,
                                        potentialReferenceFrames[i].refToFrame);
      Sophus::Vector3d dist = RefToFrame_tracked.translation() *
                              potentialReferenceFrames[i].ref->meanIdepth;
  ...
  if (bestFrame != 0) {
    if (enablePrintDebugInfo && printRelocalizationInfo)
      printf(
          "FindReferences for %d: Checked %d (%d). dist %.3f + usage %.3f = "
          "%.3f. pose discrepancy %.2f. TAKE %d!\n",
          (int)frame->id(), (int)potentialReferenceFrames.size(),
          checkedSecondary, bestDist, bestUsage, bestScore, bestPoseDiscrepancy,
          bestFrame->id());
    return bestFrame;
  } else {
    if (enablePrintDebugInfo && printRelocalizationInfo)
...
std::unordered_set<Frame*> TrackableKeyFrameSearch::findCandidates(
    Frame* keyframe, Frame*& fabMapResult_out, bool includeFABMAP,
    bool closenessTH) {
...
Frame* TrackableKeyFrameSearch::findAppearanceBasedCandidate(Frame* keyframe) {
  ...
  if (!fabMap.isValid()) {
    printf(
        "Error: called findAppearanceBasedCandidate(), but FabMap instance is "
        "not valid!\n");
    return nullptr;
  }
  ...
  if (useFabMap)
    printf(
        "Warning: Compiled without FabMap, but useFabMap is enabled... "
        "ignoring.\n");
  return nullptr;
#endif
}
