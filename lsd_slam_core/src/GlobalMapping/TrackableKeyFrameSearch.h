...
class TrackableKeyFrameSearch {
 public:
  ...
  /** Constructor. */
  TrackableKeyFrameSearch(KeyFrameGraph* graph, int w, int h,
                          Eigen::Matrix3f K);
  ...
  std::unordered_set<Frame*> findCandidates(Frame* keyframe,
                                            Frame*& fabMapResult_out,
                                            bool includeFABMAP = true,
                                            bool closenessTH = 1.0);
 ...
 private:
  /**
   * Returns a possible loop closure for the keyframe or nullptr if none is
   * found.
  ...
  findEuclideanOverlapFrames(Frame* frame, float distanceTH, float angleTH,
                             bool checkBothScales = false);
