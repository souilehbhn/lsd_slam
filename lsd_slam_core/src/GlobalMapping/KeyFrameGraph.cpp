...
KeyFrameGraph::KeyFrameGraph() : nextEdgeId(0) {
  ...
  auto solver = std::unique_ptr<LinearSolver>(new LinearSolver());
  solver->setWriteDebug(true);

  auto blockSolver =
      std::unique_ptr<BlockSolver>(new BlockSolver(std::move(solver)));
...
KeyFrameGraph::~KeyFrameGraph() {
  ...
  // WARNING: at this point, all Frames have to be deleted, otherwise it night
  // cause segfaults!
  for (FramePoseStruct* p : allFramePoses) delete p;
}
...
void KeyFrameGraph::dumpMap(std::string folder) {
  ...
  for (unsigned int i = 0; i < keyframesAll.size(); i++) {
    ...
    cv::imwrite(buf, getVarRedGreenPlot(
                         keyframesAll[i]->idepthVar(), keyframesAll[i]->image(),
  ...
  for (unsigned int i = 0; i < keyframesAll.size(); i++) {
    ...
    usedPixels[i] =
        keyframesAll[i]->numPoints / (float)keyframesAll[i]->numMappablePixels;
  }
  ...
  for (unsigned int i = 0; i < edgesAll.size(); i++) {
    ...
    usage(e->firstFrame->idxInKeyframes, e->secondFrame->idxInKeyframes) =
        e->usage;
    consistency(e->firstFrame->idxInKeyframes, e->secondFrame->idxInKeyframes) =
        e->reciprocalConsistency;
...
void KeyFrameGraph::addKeyFrame(Frame* frame) {
  if (frame->pose->graphVertex != nullptr) return;
  ...
  if (!frame->hasTrackingParent()) vertex->setFixed(true);
...
void KeyFrameGraph::calculateGraphDistancesToFrame(
    Frame* startFrame, std::unordered_map<Frame*, int>* distanceMap) {
