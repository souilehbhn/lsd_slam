...
SlamSystem::SlamSystem(int w, int h, Eigen::Matrix3f K, bool enableSLAM)
  ...
  if (w % 16 != 0 || h % 16 != 0) {
    printf(
        "image dimensions must be multiples of 16! Please crop your images / "
        "video accordingly.\n");
    assert(false);
  }
  ...
  if (SLAMEnabled) {
    trackableKeyFrameSearch =
        new TrackableKeyFrameSearch(keyFrameGraph, w, h, K);
  ...
  if (SLAMEnabled) {
    thread_constraint_search =
        boost::thread(&SlamSystem::constraintSearchThreadLoop, this);
    thread_optimization =
        boost::thread(&SlamSystem::optimizationThreadLoop, this);
  }
...
SlamSystem::~SlamSystem() {
  ...
  // make shure to reset all shared pointers to all frames before deleting the
  // keyframegraph!
...
void SlamSystem::mergeOptimizationOffset() {
  ...
  if (needPublish) publishKeyframeGraph();
}
    ...
    if (newKeyFrames.size() == 0) {
      ...
      if (keyFrameGraph->keyframesForRetrack.size() > 10) {
        std::deque<Frame*>::iterator toReTrack =
            keyFrameGraph->keyframesForRetrack.begin() +
      ...
      if (!doneSomething) {
        ...
        newKeyFrameCreatedSignal.timed_wait(
            lock, boost::posix_time::milliseconds(500));
      }
    } else {
      ...
      for (unsigned int i = 0; i < keyFrameGraph->keyframesAll.size(); i++) {
        if (keyFrameGraph->keyframesAll[i]->pose->isInGraph)
          added += findConstraintsForNewKeyFrames(
              keyFrameGraph->keyframesAll[i], false, false, 1.0);
      }
...
void SlamSystem::optimizationThreadLoop() {
  ...
  while (keepRunning) {
    ...
    if (!newConstraintAdded)
      newConstraintCreatedSignal.timed_wait(
          lock, boost::posix_time::milliseconds(
                    2000));  // slight chance of deadlock otherwise
    newConstraintAdded = false;
    lock.unlock();
...
void SlamSystem::discardCurrentKeyframe() {
  ...
  if (currentKeyFrame->idxInKeyframes >= 0) {
    printf(
        "WARNING: trying to discard a KF that has already been added to the "
        "graph... finalizing instead.\n");
...
void SlamSystem::createNewCurrentKeyframe(
  ...
  if (printPropagationStatistics) {
    ...
    data[2] = runningStats.num_prop_removed_colorDiff /
              (float)runningStats.num_prop_attempts;
  ...
  if (newReferenceKF != 0)
    ...
    if (force) {
      if (noCreate) {
        ...
        nextRelocIdx = -1;
        printf(
            "mapping is disabled & moved outside of known map. Starting "
            "Relocalizer!\n");
      } else
...
bool SlamSystem::updateKeyframe() {
  ...
  std::deque<std::shared_ptr<Frame>,
             Eigen::aligned_allocator<std::shared_ptr<Frame>>>
      references;
...
void SlamSystem::addTimingSamples() {
  ...
  float sPassed = ((now.tv_sec - lastHzUpdate.tv_sec) +
                   (now.tv_usec - lastHzUpdate.tv_usec) / 1000000.0f);
  if (sPassed > 1.0f) {
    ...
    nAvgFindReferences =
        0.8 * nAvgFindReferences + 0.2 * (nFindReferences / sPassed);
    nFindReferences = 0;

    if (trackableKeyFrameSearch != 0) {
      trackableKeyFrameSearch->nAvgTrackPermaRef =
          0.8 * trackableKeyFrameSearch->nAvgTrackPermaRef +
    ...
    if (enablePrintDebugInfo && printOverallTiming) {
      printf(
          "MapIt: %3.1fms (%.1fHz); Track: %3.1fms (%.1fHz); Create: %3.1fms "
          "(%.1fHz); FindRef: %3.1fms (%.1fHz); PermaTrk: %3.1fms (%.1fHz); "
          "Opt: %3.1fms (%.1fHz); FindConst: %3.1fms (%.1fHz);\n",
          map->msUpdate, map->nAvgUpdate, msTrackFrame, nAvgTrackFrame,
          map->msCreate + map->msFinalize, map->nAvgCreate, msFindReferences,
          nAvgFindReferences,
          trackableKeyFrameSearch != 0
              ? trackableKeyFrameSearch->msTrackPermaRef
              : 0,
          trackableKeyFrameSearch != 0
              ? trackableKeyFrameSearch->nAvgTrackPermaRef
              : 0,
...
void SlamSystem::debugDisplayDepthMap() {
  ...
  snprintf(buf1, 200,
           ...
           currentKeyFrame->numMappedOnThis, (int)unmappedTrackedFrames.size());

  snprintf(buf2, 200,
           "dens %2.0f%%; good %2.0f%%; scale %2.2f; res %2.1f/; usg %2.0f%%; "
           "Map: %d F, %d KF, %d E, %.1fm Pts",
           100 * currentKeyFrame->numPoints / (float)(width * height),
           100 * tracking_lastGoodPerBad, scale, tracking_lastResidual,
           100 * tracking_lastUsage, (int)keyFrameGraph->allFramePoses.size(),
           keyFrameGraph->totalVertices, (int)keyFrameGraph->edgesAll.size(),
...
bool SlamSystem::doMappingIteration() {
  ...
  if (trackingIsGood) {
    if (!doMapping) {
      ...
      if (displayDepthMap || depthMapScreenshotFlag) debugDisplayDepthMap();
    ...
    if (!relocalizer.isRunning) relocalizer.start(keyFrameGraph->keyframesAll);
...
float SlamSystem::tryTrackSim3(TrackingReference* A, TrackingReference* B,
                               int lvlStart, int lvlEnd, bool useSSE,
                               Sim3& AtoB, Sim3& BtoA, KFConstraintStruct* e1,
                               KFConstraintStruct* e2) {
  BtoA =
      constraintTracker->trackFrameSim3(A, B->keyframe, BtoA, lvlStart, lvlEnd);
  ...
  AtoB =
      constraintTracker->trackFrameSim3(B, A->keyframe, AtoB, lvlStart, lvlEnd);
  ...
  Matrix7x7 diffHesse =
      (AtoBInfo.inverse() +
       datimesb_db * BtoAInfo.inverse() * datimesb_db.transpose())
          .inverse();
...
void SlamSystem::testConstraint(Frame* candidate, KFConstraintStruct*& e1_out,
  ...
  float err_level3 =
      tryTrackSim3(newKFTrackingReference,
...
int SlamSystem::findConstraintsForNewKeyFrames(Frame* newKeyFrame,
  ...
  std::unordered_set<Frame*> candidates =
      trackableKeyFrameSearch->findCandidates(newKeyFrame, fabMapResult,
                                              useFABMAP, closeCandidatesTH);
  std::map<Frame*, Sim3, std::less<Frame*>,
           Eigen::aligned_allocator<std::pair<const Frame*, Sim3>>>
  ...
  Frame* parent =
      newKeyFrame->hasTrackingParent() ? newKeyFrame->getTrackingParent() : 0;
  ...
  for (Frame* candidate : candidates) {
    if (candidate->id() == newKeyFrame->id()) continue;
    if (!candidate->pose->isInGraph) continue;
    if (newKeyFrame->hasTrackingParent() &&
        candidate == newKeyFrame->getTrackingParent())
      continue;
    if (candidate->idxInKeyframes < INITIALIZATION_PHASE_COUNT) continue;
  ...
  for (Frame* candidate : candidates) {
    ...
    if (distancesToNewKeyFrame.at(candidate) < 4) continue;
  ...
  for (Frame* candidate : closeCandidates) {
    ...
    testConstraint(candidate, e1, e2,
                   candidateToFrame_initialEstimateMap[candidate],
  ...
  if (parent != 0 && forceParent) {
    KFConstraintStruct* e1 = 0;
    KFConstraintStruct* e2 = 0;
    testConstraint(parent, e1, e2, candidateToFrame_initialEstimateMap[parent],
                   100);
    if (enablePrintDebugInfo && printConstraintSearchInfo)
      printf(" PARENT (0)\n");
    ...
    if (e1 != 0) {
      ...
      printf(
          "warning: reciprocal tracking on new frame failed badly, added "
          "odometry edge (Hacky).\n");
...
bool SlamSystem::optimizationIteration(int itsPerTry, float minChange) {
  ...
  for (size_t i = 0; i < keyFrameGraph->keyframesAll.size(); i++) {
    ...
    // add error
    for (auto edge :
         keyFrameGraph->keyframesAll[i]->pose->graphVertex->edges()) {
      keyFrameGraph->keyframesAll[i]->edgeErrorSum +=
          ((EdgeSim3*)(edge))->chi2();
...
Sophus::Sim3f SlamSystem::getCurrentPoseEstimateScale() {
  ...
  if (keyFrameGraph->allFramePoses.size() > 0)
    camToWorld =
        keyFrameGraph->allFramePoses.back()->getCamToWorld().cast<float>();
  keyFrameGraph->allFramePosesMutex.unlock_shared();
  return camToWorld;
}
